 ...
 */
package hibernate.jpa.internal;
...
public class QueryImpl<X> extends AbstractQueryImpl<X>
	...
	private List<X> list() {
		...
		try {
			if(query.getReturnAliases() != null && query.getReturnAliases().length > 0){
				boolean isReturnEntity = true;
				for(String s : query.getReturnAliases()){
					if(s == null){
				...
				if(isReturnEntity){
					...
					if(queryStr.trim().length() > 0){
						String[] selects = queryStr.replace("select ", "").split(",");
						
						if(selects.length > 0){
							String entityName = "";
							if (queryStr.indexOf("where ") != -1) {
								String[] tmpStr = query.getQueryString().substring(query.getQueryString().indexOf("from ") + 5, query.getQueryString().indexOf("where ")).trim().split(" ");
								if(tmpStr.length > 0){
									entityName = tmpStr[0];
								}
							}else{
								String[] tmpStr = query.getQueryString().substring(query.getQueryString().indexOf("from ") + 5).trim().split(" ");
								if(tmpStr.length > 0){
									entityName = tmpStr[0];
								}
							}
							...
							if(entityName == ""){
								throw new IllegalArgumentException("获取到的实体名为空");
							}
							
							Class<?> clz = ClassCache.getClassByClassName(entityName.replaceAll("\"", "").trim());
