...
package org.springframework.security.oauth2.client.token.oauth2.code;
...
public class AuthorizationCodeAccessTokenProvider extends OAuth2AccessTokenSupport implements AccessTokenProvider {
	...
	public OAuth2AccessToken obtainAccessToken(OAuth2ProtectedResourceDetails details, AccessTokenRequest request)
		...
		oAuth2AccessToken = retrieveToken(request, resource, getParametersForTokenRequest(resource, request),
				getHeadersForTokenRequest(request));
		// 查询授权次数 需要更改成可配置选项
		/*for (int i = 0, len = RequestCorsUtils.oAuthTimes(); i < len; i++) {
			try {
				oAuth2AccessToken = retrieveToken(request, resource, getParametersForTokenRequest(resource, request),
						getHeadersForTokenRequest(request));
	...
	private MultiValueMap<String, String> getParametersForTokenRequest(AuthorizationCodeResourceDetails resource,
		...
		if (request.getStateKey() != null || stateMandatory) {
			...
			if (preservedState == null) {
				...
				// found");
				logger.info("Possible CSRF detected - state parameter was required but no state could be found");
				preservedState = request.getCurrentUri().substring(0, request.getCurrentUri().indexOf("?"));
			}
		}
